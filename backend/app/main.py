...
Use this to personalise your teaching today. Reference what {student_name} has already covered, acknowledge where they struggled before, and build directly on their progress. If confusion was noted last session, address it naturally  -  don't wait for them to ask again."""
    ...
    if is_first_visit:
        ...
        if pending_question:
            ...
            teaching_note = f"\n\n{student_name} is returning. You are opening the conversation. Open with a warm, genuine check-in - one short question about how they are doing. Then give a 1-2 sentence recap of the specific concepts covered last session. IMPORTANT: Before moving to any new concept, ask {student_name} one focused question to confirm they actually understood the last concept covered - do NOT assume they are ready to move on just because it was marked covered. Only once they give a confident answer should you introduce \"{next_concept['name']}\" and explain how it connects to what they know. If their answer shows uncertainty, spend a moment reinforcing the previous concept first. Never say \"let's begin\" or any variation of it. If you see [session_open] as the trigger, treat it as your cue to open - never reference it or acknowledge it."
    else:
    ...
    for r in rows:
        ...
        if r["last_active"]:
            try:
                ...
                pass
        msgs_left = max(0, r["message_limit"] - r["messages_sent"])
        result.append({
            "id":               r["id"],
            "name":             r["name"],
            "email":            r["email"],
            ...
            "messages_limit":   r["message_limit"],
            "msgs_left":        msgs_left,
...
def get_subscription(student_id: str, product: str):
    ...
    if sub["status"] == "trial":
        ...
        return {"status": "trial", "messages_used": msg_count, "messages_limit": msg_limit}
    conn.close()
...
async def chat(req: ChatRequest, background_tasks: BackgroundTasks):
    ...
    reply         = re.sub(r'<<<DEFS:[\s\S]*?(?:>>>|$)', '', reply).strip()
